...
library Groth16AltBN128 {
    ...
    //     uint256[4] Minus_Gamma; // slots 0x06, 0x07, 0x08, 0x09
    //     uint256[4] Minus_Delta; // slots 0x0a, 0x0b, 0x0c, 0x0d
    //     uint256[] ABC;          // slot 0x0e (each entry uses 2 words)
    ...
    function _verify(
        ...
        uint256 numInputs = ((vk.length - 0x0e) / 2) - 1;
        ...
        assembly {
            ...
            let abc_slot_num := add(vk_slot_num, 0x0e)
        ...
        //   e(Proof.A, Proof.B) =
        //       e(vk.Alpha, vk.Beta) * e(vk_x_gamma, vk.Gamma) * e(Proof.C, vk.Delta)
        // where:
        ...
        // check of the form:
        //
        //   e(vk_x_gamma, vk.Minus_Gamma) * e(vk.Alpha, vk.Minus_Beta) *
        //       e(negate(Proof.A), Proof.B) * e(Proof.C, vk.Minus_Delta) == 1
        //
        // See Pairing.pairing(). Note terms have been re-ordered since vk_x_gamma is
        // already at offset 0x00. Memory is laid out:
        //
        //   0x0300
        //   0x0280 - vk.Minus_Delta in G2
        //   0x0240 - proof.C in G1
        //   0x01c0 - Proof.B in G2
        //   0x0180 - Proof.A in G1
        //   0x0100 - vk.Minus_Beta in G2
        //   0x00c0 - vk.Alpha in G1
        //   0x0040 - vk.Minus_Gamma in G2 
        //   0x0000 - vk_x_gamma in G1  (Already present, by the above)

        assembly {
            // Write vk.Minus_Gamma, from offset 0x40.

            mstore(add(pad, 0x040),sload(add(vk_slot_num, 6)))
            mstore(add(pad, 0x060),sload(add(vk_slot_num, 7)))
            mstore(add(pad, 0x080),sload(add(vk_slot_num, 8)))
            mstore(add(pad, 0x0a0),sload(add(vk_slot_num, 9)))
            ...
            // // Proof.C and vk.Minus_Delta from offset 0x240.
            mstore(add(pad, 0x240), mload(add(proof, 0xc0)))
            mstore(add(pad, 0x260), mload(add(proof, 0xe0)))
            mstore(add(pad, 0x280), sload(add(vk_slot_num, 0x0a)))
            mstore(add(pad, 0x2a0), sload(add(vk_slot_num, 0x0b)))
            mstore(add(pad, 0x2c0), sload(add(vk_slot_num, 0x0c)))
            mstore(add(pad, 0x2e0), sload(add(vk_slot_num, 0x0d)))
